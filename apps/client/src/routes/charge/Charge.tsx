...
async function chargeAction(
	_prevState: ChargeFormState | null,
	formData: FormData,
	...
	try {
		...
		return {
			error:
				err.response?.data?.error?.message || 'Error al recargar billetera',
		}
	}
}
	...
	return (
		<div className={styles.container}>
			<div className={styles.card}>
				...
				<h1 className={styles.title}>Recargar Billetera</h1>
				...
				{state?.success && state.data && (
					<div className={styles.successMessage}>
						<div className={styles.paymentResult}>
							<div className={styles.successTitle}>¡Recarga exitosa!</div>
							<div
								className={`${styles.resultValue} ${styles.resultValueTransactionId}`}
							>
								{state.data.transactionId}
							</div>
							<div className={styles.resultLabel}>Monto recargado:</div>
				...
				<form action={formAction} className={styles.form} autoComplete="off">
					...
					<div className={styles.buttonGroup}>
						...
						>
							{isPending ? 'Recargando...' : 'Recargar Billetera'}
						</button>
					</div>
				</form>
			</div>
