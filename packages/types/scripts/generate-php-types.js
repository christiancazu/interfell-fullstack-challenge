...
function convertType(tsType) {
	...
	// Si es un tipo personalizado, lo dejamos como está
	return tsType
}
...
function parseInterface(content, interfaceName) {
	...
	// Separar líneas y procesar cada una
	const lines = body
		.split('\n')
		.map((line) => line.trim())
	...
	for (const line of lines) {
		// Verificar si es un método (tiene paréntesis)
...
`
	...
	if (isRepository || methods.length > 0) {
		// Para repositories e interfaces con métodos, crear una interfaz PHP
		phpCode += `/**
 ...
 */
function main() {
	console.log('🚀 Generating PHP types from TypeScript definitions...\n')
	...
	console.log('\n✅ PHP types generation completed!')
}
